...
var residentInstallPromptEvent = null;

window.addEventListener("beforeinstallprompt", function (event) {
  residentInstallPromptEvent = event;
...
function getResidentAccessShareUrl() {
  try {
    const current = new URL(window.location.href);
    const token = (current.searchParams.get("rt") || "").trim();
    if (token) {
      const clean = new URL(getPageBaseUrl());
      clean.searchParams.set("rt", token);
...
function setResidentSaveButtonSuccess() {
  const button = document.getElementById("residentSaveAccessBtn");
  if (!button) return;
  const textEl = button.querySelector("span:last-child") || button;
  const original = button.dataset.originalText || "Зберегти доступ";
  button.dataset.originalText = original;
  textEl.textContent = "✓ Доступ збережено";
  window.clearTimeout(button._residentSaveTimer);
  button._residentSaveTimer = window.setTimeout(function () {
    textEl.textContent = original;
  }, 2200);
}

function showResidentManualCopy(container, url) {
  const target = container ? container.querySelector(".resident-save-access-manual") : null;
  if (!target) return;
  target.hidden = false;
...
function closeResidentSaveAccessPanel() {
  const overlay = document.querySelector(".resident-save-access-overlay");
  if (overlay) overlay.remove();
  document.removeEventListener("keydown", handleResidentSaveAccessKeydown);
}

function handleResidentSaveAccessKeydown(event) {
  if (event.key === "Escape") closeResidentSaveAccessPanel();
}

function bindResidentSaveAccessButton() {
  const button = document.getElementById("residentSaveAccessBtn");
  if (!button || button.dataset.bound === "1") return;
  button.dataset.bound = "1";
  button.addEventListener("click", function () {
    openResidentSaveAccessPanel();
  });
}

function openResidentSaveAccessPanel() {
  closeResidentSaveAccessPanel();

  const url = getResidentAccessShareUrl();
  const overlay = document.createElement("div");
  overlay.className = "resident-save-access-overlay";
  overlay.setAttribute("role", "presentation");

  const panel = document.createElement("section");
  panel.className = "resident-save-access-panel";
  panel.setAttribute("role", "dialog");
  panel.setAttribute("aria-modal", "true");
  panel.setAttribute("aria-labelledby", "resident-save-access-title");
  ...
  panel.innerHTML = `
    ...
    <h3 id="resident-save-access-title">Збережіть доступ до кабінету</h3>
    <p>Це індивідуальне посилання на ваш особовий рахунок.<br>Збережіть його, щоб наступного разу швидко відкрити кабінет.</p>
    ...
    <div class="resident-save-access-manual" hidden>
      <div>Скопіюйте посилання вручну:</div>
      <input type="text" readonly value="${escapeHtml(url)}">
    </div>
  `;

  panel.addEventListener("click", async function (event) {
    const closeBtn = event.target.closest(".resident-save-access-close");
    if (closeBtn) {
      closeResidentSaveAccessPanel();
      return;
    }

    const actionBtn = event.target.closest(".resident-save-access-action");
    if (!actionBtn) return;
    ...
    if (action === "copy") {
      ...
      if (copied) {
        showMessage("Посилання скопійовано. Збережіть його у зручному місці.");
        setResidentSaveButtonSuccess();
      } else {
        showResidentManualCopy(panel, url);
      }
      return;
    }
    ...
    if (action === "install" && residentInstallPromptEvent) {
      const promptEvent = residentInstallPromptEvent;
      residentInstallPromptEvent = null;
      try {
        promptEvent.prompt();
        ...
        showMessage("Відкрийте меню браузера та оберіть «Додати на головний екран»", "warn");
      }
    }
  });

  overlay.addEventListener("click", function (event) {
    if (event.target === overlay) closeResidentSaveAccessPanel();
  });

  overlay.appendChild(panel);
  document.body.appendChild(overlay);
  document.addEventListener("keydown", handleResidentSaveAccessKeydown);
  renderLucideIcons(panel);

  const firstAction = panel.querySelector(".resident-save-access-action");
...
function ensureCourtAccountButton(accountId, accountMeta, accountData, paymentData, isResidentMode) {
  const host = document.querySelector("#header .buttons-container");
  if (!host) return;

  if (typeof hasDocumentSectionAccess !== "function" || !hasDocumentSectionAccess("court_claims")) {
    document.getElementById("courtAccountDocBtn")?.remove();
    if (typeof loadDocumentSectionAccess === "function") {
      loadDocumentSectionAccess().then(function (access) {
        if (access && access.has && access.has("court_claims")) {
          ensureCourtAccountButton(accountId, accountMeta, accountData, paymentData, isResidentMode);
        }
      });
    }
    return;
  ...
  const totalDebt = calculateCourtBalance(accountData, paymentData, null, null).debtEnd;
  if (isResidentMode || totalDebt < 1000) {
    document.getElementById("courtAccountDocBtn")?.remove();
    return;
  }

  let button = document.getElementById("courtAccountDocBtn");
  if (!button) {
    button = document.createElement("button");
    button.type = "button";
    button.id = "courtAccountDocBtn";
    button.className = "xls-button court-doc-button";
    button.title = "Документ для суду";
    button.setAttribute("aria-label", button.title);
    button.textContent = "Суд";
    host.appendChild(button);
  }

  button.onclick = async function () {
    const options = await showCourtAccountDialog(accountId, accountData);
    if (!options) return;
    button.disabled = true;
    try {
      await exportCourtAccountWord(accountId, accountMeta || {}, accountData || {}, paymentData || {}, options);
    } catch (err) {
      console.error(err);
      showMessage(`Не вдалося сформувати документ для суду: ${err && err.message ? err.message : err}`, "err");
    } finally {
...
function courtMonthCode(year, month) {
  return Number(year) * 12 + Number(month);
}

function courtMonthFromCode(code) {
  const num = Number(code);
  const year = Math.floor((num - 1) / 12);
  const month = num - year * 12;
  return { year, month };
}

function courtMonthInputValue(code) {
  const m = courtMonthFromCode(code);
  return `${m.year}-${String(m.month).padStart(2, "0")}`;
}

function courtMonthInputToCode(value) {
  const match = String(value || "").match(/^(\d{4})-(\d{2})$/);
  if (!match) return null;
  return courtMonthCode(Number(match[1]), Number(match[2]));
}

function courtMonthLabel(code) {
  const m = courtMonthFromCode(code);
  return `${getMonthName(m.month)} ${m.year}`;
}

function courtMoney(value) {
  return `${normalizeMoney(value).toFixedWithComma()} грн`;
}

function courtMoneyNumber(value) {
  return normalizeMoney(value).toFixedWithComma();
}

function courtPaymentSum(payment) {
  return Number(payment && payment.sum) || 0;
}

function courtSelectedServiceIds(options) {
  const ids = Array.isArray(options && options.services) ? options.services : [];
  return new Set(ids.map(function (id) { return String(id); }));
}

function courtAllMonthCodes(accountData, paymentData) {
  const set = new Set();
  [accountData || {}, paymentData || {}].forEach(function (source) {
    Object.keys(source || {}).forEach(function (year) {
      Object.keys(source[year] || {}).forEach(function (month) {
        const code = courtMonthCode(year, month);
        if (Number.isFinite(code)) set.add(code);
      });
    });
  });
...
function courtChargesForMonth(accountData, code, selectedServices) {
  const m = courtMonthFromCode(code);
  const transactions = (((accountData || {})[m.year] || {})[m.month]) || {};
  const out = {};
  let total = 0;
  Object.keys(transactions || {}).forEach(function (serviceId) {
    const sid = String(serviceId);
    if (selectedServices && !selectedServices.has(sid)) return;
    const value = Number(transactions[serviceId]) || 0;
    out[sid] = value;
    total += value;
  });
  return { byService: out, total };
}

function courtPaymentsForMonth(paymentData, code) {
  const m = courtMonthFromCode(code);
  const items = ((((paymentData || {})[m.year] || {})[m.month]) || []).filter(Boolean);
  const total = items.reduce(function (sum, payment) {
    return sum + courtPaymentSum(payment);
  }, 0);
  return { items, total };
}

function calculateCourtBalance(accountData, paymentData, startCode, selectedServices) {
  const allCodes = courtAllMonthCodes(accountData, paymentData);
  let debtStart = 0;
  let balance = 0;
  allCodes.forEach(function (code) {
    const charge = courtChargesForMonth(accountData, code, selectedServices).total;
    const paid = courtPaymentsForMonth(paymentData, code).total;
    if (startCode != null && code < startCode) {
      debtStart += charge - paid;
    }
    balance += charge - paid;
  });
  return { debtStart: normalizeMoney(debtStart), debtEnd: Math.max(0, normalizeMoney(balance)) };
}

function courtAvailableServices(accountData) {
  const ids = new Set();
  Object.keys(accountData || {}).forEach(function (year) {
    Object.keys(accountData[year] || {}).forEach(function (month) {
      Object.keys(accountData[year][month] || {}).forEach(function (serviceId) {
  ...
  return Array.from(ids).sort(function (a, b) {
    ...
    return { id, name: (us && us[id]) || `Нарахування ${id}` };
  });
}

function showCourtAccountDialog(accountId, accountData) {
  return new Promise(function (resolve) {
    const monthCodes = courtAllMonthCodes(accountData, {});
    const lastCode = monthCodes.length ? monthCodes[monthCodes.length - 1] : courtMonthCode(new Date().getFullYear(), new Date().getMonth() + 1);
    const threeYearsStart = lastCode - 35;
    const services = courtAvailableServices(accountData);
    const defaultServices = new Set(["1", "7"]);
    const overlay = document.createElement("div");
    overlay.className = "court-dialog-overlay";
    overlay.innerHTML = `
      <form class="court-dialog" data-court-dialog>
        <h3>Документ для суду</h3>
        <fieldset>
          <legend>Період</legend>
          <label><input type="radio" name="period" value="3y" checked> останні 3 роки</label>
          <label><input type="radio" name="period" value="all"> всі дані</label>
          <label><input type="radio" name="period" value="from"> починаючи з місяця <input type="month" name="fromMonth" value="${escapeHtml(courtMonthInputValue(threeYearsStart))}"></label>
        </fieldset>
        <fieldset>
          <legend>Розрахунок заборгованості</legend>
          <label><input type="radio" name="mode" value="period" checked> неоплачене за вибраний період, борг на початок періоду = 0</label>
          <label><input type="radio" name="mode" value="withOpening"> нараховане за вибраний період, з показом вхідного боргу</label>
        </fieldset>
        <fieldset>
          <legend>Додаткові нарахування</legend>
          <label><input type="checkbox" name="annual3" checked> нарахувати 3% річних</label>
          <label><input type="checkbox" name="inflation" checked> нарахувати індекс інфляції</label>
        </fieldset>
        <fieldset class="court-services">
          <legend>Види нарахувань</legend>
          ${services.map(function (service) {
            const checked = defaultServices.has(String(service.id)) ? "checked" : "";
            return `<label><input type="checkbox" name="service" value="${escapeHtml(service.id)}" ${checked}> <span>${escapeHtml(service.id)}: ${escapeHtml(service.name)}</span></label>`;
          }).join("")}
        </fieldset>
        <div class="court-dialog-actions">
          <button type="button" data-court-cancel>Скасувати</button>
          <button type="submit">Word</button>
        </div>
      </form>
    `;
    if (!document.getElementById("court-dialog-style")) {
      const style = document.createElement("style");
      style.id = "court-dialog-style";
      style.textContent = `
        .court-dialog-overlay{position:fixed;inset:0;z-index:10000;display:flex;align-items:center;justify-content:center;background:rgba(15,23,42,.36)}
        .court-dialog{width:min(760px,calc(100vw - 28px));max-height:min(88vh,820px);overflow:auto;background:#fff;border:1px solid #cfd6df;border-radius:10px;box-shadow:0 18px 42px rgba(15,23,42,.24);padding:18px;font-family:Arial,sans-serif;color:#111}
        .court-dialog h3{margin:0 0 12px;font-size:20px}
        .court-dialog fieldset{border:1px solid #d7dde6;border-radius:8px;margin:10px 0;padding:10px 12px}
        .court-dialog legend{font-weight:700;padding:0 6px}
        .court-dialog label{display:block;margin:8px 0;line-height:1.35}
        .court-dialog input[type="month"]{margin-left:8px;padding:5px}
        .court-services{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0 14px}
        .court-services legend{grid-column:1/-1}
        .court-dialog-actions{display:flex;justify-content:flex-end;gap:10px;margin-top:14px}
        .court-dialog button{border:1px solid #aeb7c3;border-radius:8px;background:#fff;padding:8px 16px;cursor:pointer}
        .court-dialog button[type="submit"]{background:#1f4b7a;border-color:#1f4b7a;color:#fff}
        @media(max-width:700px){.court-services{display:block}}
      `;
    ...
    overlay.querySelector("[data-court-cancel]").addEventListener("click", function () {
      close(null);
    });
    overlay.querySelector("[data-court-dialog]").addEventListener("submit", function (event) {
      event.preventDefault();
      const form = event.currentTarget;
      const checkedServices = Array.from(form.querySelectorAll('input[name="service"]:checked')).map(function (input) {
        return input.value;
      });
      if (!checkedServices.length) {
        showMessage("Оберіть хоча б один вид нарахувань", "warn");
        return;
      }
      close({
        period: form.period.value,
        fromCode: courtMonthInputToCode(form.fromMonth.value),
        lastCode,
        mode: form.mode.value,
        annual3: !!form.annual3.checked,
        inflation: !!form.inflation.checked,
...
function buildCourtRows(accountId, accountMeta, accountData, paymentData, options) {
  ...
  for (let code = startCode; code <= endCode; code += 1) {
    const charge = courtChargesForMonth(accountData, code, selected);
    const payments = courtPaymentsForMonth(paymentData, code);
    balance += charge.total - payments.total;
    chargeTotal += charge.total;
    paidTotal += payments.total;
    Object.keys(charge.byService).forEach(function (serviceId) {
    ...
    rows.push({
      code,
      label: courtMonthLabel(code),
      charges: charge.byService,
      chargeTotal: charge.total,
  ...
  return {
    ...
    paidTotal,
    chargeTotal,
    openingDebt: normalizeMoney(openingDebt),
    closingDebt: Math.max(0, normalizeMoney(balance)),
    startCode,
    endCode
  };
}
...
function courtBuildLots(rows, options) {
  ...
  rows.forEach(function (row) {
    ...
    let paid = row.paid;
    for (let i = 0; i < lots.length && paid > 0; i += 1) {
      const take = Math.min(lots[i].rest, paid);
      lots[i].rest = normalizeMoney(lots[i].rest - take);
      paid = normalizeMoney(paid - take);
    }
  });
  return lots.filter(function (lot) { return lot.rest > 0.005; });
}

function courtDebtByService(lots) {
  const out = {};
  (lots || []).forEach(function (lot) {
    const sid = String(lot.serviceId || "");
    out[sid] = normalizeMoney((out[sid] || 0) + (Number(lot.rest) || 0));
  });
  return out;
}

function calculateCourtAnnual3(lots, asOfDate) {
  const end = asOfDate || new Date();
  return normalizeMoney(lots.reduce(function (sum, lot) {
    const start = new Date(lot.dueDate.getFullYear(), lot.dueDate.getMonth(), lot.dueDate.getDate() + 1);
    const days = Math.max(0, Math.floor((end - start) / 86400000));
    return sum + lot.rest * 0.03 * days / 365;
  }, 0));
}

function buildCourtAnnual3Rows(lots, asOfDate) {
  const end = asOfDate || new Date();
  return (lots || []).map(function (lot) {
    const start = new Date(lot.dueDate.getFullYear(), lot.dueDate.getMonth(), lot.dueDate.getDate() + 1);
    const days = Math.max(0, Math.floor((end - start) / 86400000));
    const amount = normalizeMoney(lot.rest * 0.03 * days / 365);
    return {
      label: lot.label,
      rest: normalizeMoney(lot.rest),
      startDate: formatDate(start, "DD.MM.YYYY"),
      days,
      amount
    };
  }).filter(function (row) {
    return row.rest > 0.005 && row.days > 0 && row.amount > 0.005;
  });
}

async function fetchCourtInflationRates() {
  const map = new Map();
  if (!window.client) throw new Error("Supabase SDK unavailable");
  const { data, error } = await client
    .from("inflation_indices")
    .select("month_code,rate")
    .order("month_code", { ascending: true });
  if (error) throw error;
  (data || []).forEach(function (row) {
    const code = Number(row.month_code);
    const rate = Number(row.rate);
    if (Number.isFinite(code) && Number.isFinite(rate) && rate > 0) {
      map.set(code, rate);
    }
  });
  if (!map.size) throw new Error("індекси інфляції не завантажені в базу");
  return map;
}
...
function calculateCourtInflation(lots, rates, asOfDate) {
  ...
  const total = lots.reduce(function (sum, lot) {
    ...
    return sum + Math.max(0, lot.rest * (factor - 1));
  }, 0);
  return { total: normalizeMoney(total), missing };
}

function buildCourtInflationRows(lots, rates, asOfDate) {
  const end = asOfDate || new Date();
  const endCode = courtMonthCode(end.getFullYear(), end.getMonth() + 1);
  return (lots || []).map(function (lot) {
    const startCode = courtMonthCode(lot.dueDate.getFullYear(), lot.dueDate.getMonth() + 2);
    ...
    const amount = normalizeMoney(Math.max(0, lot.rest * (factor - 1)));
    return {
      label: lot.label,
      rest: normalizeMoney(lot.rest),
      period: startCode <= endCode ? `${courtMonthLabel(startCode)} - ${courtMonthLabel(endCode)}` : "",
      index: factor,
      missing,
      amount
    };
...
function courtPaymentText(payments) {
  if (!payments || !payments.length) return "";
  return payments.map(function (p) {
    const date = p.date || p.dt || "";
    return `${date ? date + " " : ""}${courtMoney(courtPaymentSum(p))}`;
  }).join("; ");
}

function courtOrgKind(value) {
...
function courtTemplateNeedTpv(options) {
  const selected = courtSelectedServiceIds(options);
  return selected.has("7");
}

function courtTemplateMatchesKind(doc, kind) {
  const hay = `${doc.doc_number || ""} ${doc.summary || ""} ${doc.body || ""}`.toLowerCase();
  const isOsbb = /осбб/.test(hay);
  const isCoop = /(^|[^а-яіїєґ])(жк|жбк)([^а-яіїєґ]|$)/i.test(hay) || /кооператив/.test(hay);
  if (kind === "osbb" && isCoop && !isOsbb) return false;
  if (kind === "coop" && isOsbb && !isCoop) return false;
  return true;
}

function courtTemplateIsTpv(doc) {
  const hay = `${doc.doc_number || ""} ${doc.summary || ""} ${doc.legacy_code || ""}`.toLowerCase();
  return /тпв|сміт|мусор|побутов/.test(hay);
}

async function fetchCourtTemplate(homeCode, orgName, options) {
  if (!window.client || !homeCode) return null;
  try {
    const { data, error } = await client
      .from("outgoing_documents")
      .select("*")
      .eq("home_code", String(homeCode))
      .eq("is_template", true);
    if (error) throw error;
    const kind = courtOrgKind(orgName);
    const needTpv = courtTemplateNeedTpv(options);
    const list = (data || []).filter(function (doc) {
      return courtTemplateMatchesKind(doc || {}, kind);
    });
    const main = list.find(function (doc) {
      return !courtTemplateIsTpv(doc) && /основ|голов|исков|позов|заяв/i.test(`${doc.doc_number || ""} ${doc.summary || ""}`);
    }) || list.find(function (doc) {
      return !courtTemplateIsTpv(doc);
    }) || null;
    const tpv = needTpv ? (list.find(function (doc) {
      return courtTemplateIsTpv(doc) && /тпв|сміт|мусор|побутов/i.test(`${doc.doc_number || ""} ${doc.summary || ""}`);
    }) || list.find(function (doc) {
      return courtTemplateIsTpv(doc);
    }) || null) : null;
    if (!main && !tpv) return null;
    if (!main) return { body: tpv.body || "" };
    const mainBody = String(main.body || "");
    const tpvBody = tpv ? String(tpv.body || "") : "";
    if (!tpvBody) {
      return { body: mainBody.replace(/\{(?:ТПВ|tpv|garbage|сміття)\}/gi, "") };
    }
    if (/\{(?:ТПВ|tpv|garbage|сміття)\}/i.test(mainBody)) {
      return { body: mainBody.replace(/\{(?:ТПВ|tpv|garbage|сміття)\}/gi, tpvBody) };
    }
    return { body: `${mainBody}\n\n${tpvBody}` };
  } catch (err) {
    console.warn("Не вдалося завантажити шаблон судового документа", err);
    return null;
  }
}

function courtFirstValue() {
...
function courtCleanFlatAddress(value) {
  return String(value || "")
    .replace(/\s*\/\s*$/, "")
    .replace(/\s+\/\s*$/, "")
    .replace(/\s*,\s*кв\.\/прим\.\s*[^,]+$/i, "")
    .replace(/\s*,\s*кв\.?\s*[^,]+$/i, "")
    .replace(/\s+\/\s+.*$/, "")
    .replace(/\s{2,}/g, " ")
    .trim();
...
function courtDebtSummaryMarkdown(calc, annual3, inflation, grandTotal, debtByService, options, courtFeeInfo) {
  ...
  if (housingDebt > 0.005) {
    lines.push(`* заборгованість зі сплати внесків та платежів на утримання будинку — **${courtMoney(housingDebt)}**;`);
  }
  if (selected.has("7") && tpvDebt > 0.005) {
    lines.push(`* заборгованість зі сплати платежів за вивезення побутових відходів — **${courtMoney(tpvDebt)}**;`);
  }
  if (options && options.inflation) {
    lines.push(`* інфляційні втрати — **${courtMoney(inflation && inflation.total)}**;`);
  }
...
function courtClaimsRequestsMarkdown(accountMeta, calc, annual3, inflation, grandTotal, totalWithCourtFee, debtByService, homeName, okpo, addressText, options, courtFeeInfo) {
  ...
  const orgFull = homeName;
  const debtText = orgKind === "osbb"
  ...
  const base = `з ${fio} на користь ${orgFull}, код ЄДРПОУ ${okpo}`;
  ...
  if (options && options.inflation && normalizeMoney(inflation && inflation.total) > 0.005) {
    items.push(`${items.length + 1}. **Стягнути ${base} інфляційні втрати у розмірі ${courtMoney(inflation.total)}.**`);
  }
...
function courtPlaceholderMap(accountId, accountMeta, calc, annual3, inflation, grandTotal, totalWithCourtFee, homeName, okpo, addressText, homeMeta, debtByService, options, courtFeeInfo) {
  ...
  return {
    ...
    okpo,
    fio: accountMeta.fio || "",
    adr: courtCleanFlatAddress(addressText),
    ...
    "телефон": courtFirstValue(home.tel, home.phone),
    "e-mail": courtFirstValue(home.email, home.mail),
    "піб": headName,
    "головаFull": headName,
    "головаfull": headName,
    "ГоловаFull": headName,
...
function courtReplaceTemplateText(text, map) {
  let out = String(text || "");
  ...
  out = out
    .replace(/\[ДАТА ПОЧАТКУ\]/gi, map["дата початку"] || "")
    .replace(/\[ДАТА ЗАКІНЧЕННЯ\]/gi, map["дата закінчення"] || "")
    .replace(/\[ДАТА РОЗРАХУНКУ\]/gi, map["дата розрахунку"] || "")
    .replace(/\[ДАТА\]/gi, map["дата"] || "")
    .replace(/\[АДРЕСА ОСББ\]/gi, map["адреса осбб"] || "")
    .replace(/\[АДРЕСА КООПЕРАТИВУ\]/gi, map["адреса кооперативу"] || "")
    .replace(/\[ТЕЛЕФОН\]/gi, map["телефон"] || "")
    .replace(/\[E-MAIL\]/gi, map["e-mail"] || "")
    .replace(/\[ПІБ\]/gi, map["піб"] || "")
    .replace(/\[СУМА ОСНОВНОЇ ЗАБОРГОВАНОСТІ\]/gi, map["сума основної заборгованості"] || "")
...
function courtInlineMarkdown(text) {
  return escapeHtml(text).replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>");
}

function exportCourtDownload(html, fileName) {
...
async function exportCourtAccountWord(accountId, accountMeta, accountData, paymentData, options) {
  const selected = courtSelectedServiceIds(options);
  const calc = buildCourtRows(accountId, accountMeta, accountData, paymentData, options);
  const services = courtAvailableServices(accountData).filter(function (service) {
    return selected.has(String(service.id));
  });
  ...
  const okpo = homeMeta.okpo || homeMeta.okpo1 || homeMeta.code || homeCode || "";
  const baseAddress = courtCleanFlatAddress(homeMeta.adr || homeMeta.adrfull || homeMeta.adrlong || document.getElementById("adr")?.textContent || accountMeta.adr || accountMeta.address || "");
  const addressText = [baseAddress, accountMeta.kv ? `кв. ${accountMeta.kv}` : ""].filter(Boolean).join(", ");
  const fio = accountMeta.fio || "";
  const accountNo = accountMeta.ls || accountMeta.kv || accountId;
  const showChargeTotal = services.length > 1;
  const serviceHead = services.map(function (service) {
...
<div class="meta">
  ...
  <div><strong>Період:</strong> ${escapeHtml(courtMonthLabel(calc.startCode))} - ${escapeHtml(courtMonthLabel(calc.endCode))}</div>
  <div><strong>Боржник:</strong> ${escapeHtml(fio)}</div>
  <div><strong>Адреса:</strong> ${escapeHtml(addressText)}</div>
</div>
<h2>Види нарахувань, включені до розрахунку</h2>
<ul>${includedServices}</ul>
${calc.openingDebt ? `<p><strong>Заборгованість на початок періоду:</strong> ${courtMoney(calc.openingDebt)}</p>` : ""}
<h2>Розрахунок нарахувань, оплат і заборгованості</h2>
<table>
  <thead>
    <tr>
      <th style="width:28mm">Місяць</th>
      ${serviceHead}
      ${showChargeTotal ? '<th style="width:28mm">Нараховано разом</th>' : ""}
      <th style="width:45mm">Оплати</th>
      <th style="width:30mm">Борг на кінець</th>
    </tr>
  </thead>
...
function renderContactGroups(contactGroups, root) {
  ...
  const cardsHtml = groups.map(function (group, idx) {
    ...
    const phoneLines = (group.phones || []).map(function (phone) {
      return `
        <a href="tel:${phone.phone}" class="resident-contact-line resident-contact-line-phone resident-contact-line-clickable">
          <span class="resident-contact-line-main">
            <span class="resident-inline-icon" aria-hidden="true" data-lucide="phone"></span>
            <span class="resident-contact-phone-text">${escapeHtml(phone.displayPhone || phone.phone || "")}</span>
          </span>
        </a>
      `;
    }).join("");
    ...
    const emailLines = (group.emails || []).map(function (email) {
      return `
        <a href="mailto:${encodeURIComponent(email)}" class="resident-contact-line resident-contact-line-email resident-contact-line-clickable">
          <span class="resident-contact-line-main">
            <span class="resident-inline-icon" aria-hidden="true" data-lucide="mail"></span>
            <span class="resident-contact-email-text">${escapeHtml(email)}</span>
          </span>
        </a>
      `;
    }).join("");
    ...
    const actionTelegram = tgUsername
      ...
      : (phoneWithTelegram
        ? `<a href="https://t.me/${phoneWithTelegram.phone}" class="resident-contact-action-btn" target="_blank" rel="noopener noreferrer"><span class="resident-inline-icon resident-inline-icon-img" aria-hidden="true"><img src="img/tg.png" alt="" class="resident-messenger-icon-img"></span><span>Telegram</span></a>`
        : "");
    const actionBar = (actionViber || actionTelegram)
      ? `<div class="resident-contact-actions">${actionViber}${actionTelegram}</div>`
      : "";
...
function renderResidentUsefulLinks(root) {
  if (!root) return;
  const cards = Array.isArray(window.RESIDENT_USEFUL_LINKS)
    ? window.RESIDENT_USEFUL_LINKS
    : [];
  if (!cards.length) {
    root.innerHTML = "";
    root.style.display = "none";
  ...
  const phoneToHref = function (rawPhone) {
  ...
  const getLinkIconHtml = function (label) {
    const key = String(label || "").toLowerCase().trim();
    if (key === "viber") {
      return '<span class="useful-btn-icon useful-btn-icon-img" aria-hidden="true"><img src="img/viber.png" alt=""></span>';
    }
    if (key === "telegram") {
      return '<span class="useful-btn-icon useful-btn-icon-img" aria-hidden="true"><img src="img/tg.png" alt=""></span>';
    }
    if (key.indexOf("mail") >= 0 || key.indexOf("e-mail") >= 0) {
      return '<span class="useful-btn-icon" aria-hidden="true" data-lucide="mail"></span>';
    }
    if (key.indexOf("контакти") >= 0) {
      return '<span class="useful-btn-icon" aria-hidden="true" data-lucide="contact-round"></span>';
    }
    if (key.indexOf("передача") >= 0 || key.indexOf("показ") >= 0) {
      return '<span class="useful-btn-icon" aria-hidden="true" data-lucide="gauge"></span>';
    }
    return '<span class="useful-btn-icon" aria-hidden="true" data-lucide="external-link"></span>';
  };

  const getUsefulCardHomeUrl = function (links) {
    const source = (Array.isArray(links) ? links : []).find(function (link) {
      const href = String(link && link.href || "").trim();
      return /^https:/i.test(href) && !/^https:\/\/t\.me\//i.test(href);
    });
    if (!source) return "";

    try {
      const url = new URL(String(source.href || "").trim());
      if (url.protocol !== "https:") return "";
      let host = url.host;
      host = host.replace(/^(my|billing|pa)\./i, "");
      return `https://${host}`;
    } catch (e) {
      return "";
    }
  ...
  const calcCardWeight = function (card) {
    const phones = Array.isArray(card && card.phones) ? card.phones : [];
    const phoneLengths = phones.map(function (p) { return String(p || "").trim().length; });
    const maxPhoneLen = phoneLengths.length ? Math.max.apply(null, phoneLengths) : 0;
    const linksCount = Array.isArray(card && card.links) ? card.links.length : 0;
    const textWeight =
      String(card && card.name || "").length +
      String(card && card.title || "").length +
      String(card && card.subtitle || "").length;
    return phones.length * 10 + maxPhoneLen + linksCount * 3 + Math.round(textWeight * 0.15);
  };

  const renderedCards = cards.map(function (card, index) {
    return {
      index,
      card,
      weight: calcCardWeight(card)
    };
  });

  let wideTarget = 0;
  if (renderedCards.length >= 5) {
    wideTarget = 2;
  } else if (renderedCards.length === 4) {
    wideTarget = 2;
  }

  const sortedByWeight = renderedCards.slice().sort(function (a, b) {
    if (b.weight !== a.weight) return b.weight - a.weight;
    return a.index - b.index;
  });
  const wideIndexSet = new Set(sortedByWeight.slice(0, wideTarget).map(function (item) { return item.index; }));

  const renderCard = function (item, localIndex, className) {
    const card = item.card;
    const cardClass = `useful-card ${className}`;
    const phonesHtml = (card.phones || []).map(function (phone) {
      const href = phoneToHref(phone);
      const safePhone = escapeHtml(String(phone || "").trim());
      const body = `
          <span class="useful-phone-icon" aria-hidden="true" data-lucide="phone"></span>
          <span>${safePhone}</span>
      `;
      if (href) {
        return `<a class="useful-phone-row" href="${href}">${body}</a>`;
      }
      return `<div class="useful-phone-row">${body}</div>`;
    }).join("");

    const linksHtml = (card.links || []).map(function (link) {
      const iconHtml = getLinkIconHtml(link.label);
      return `
        <a class="useful-link-btn" href="${escapeHtml(link.href || "#")}" target="_blank" rel="noopener noreferrer">
          ${iconHtml}
          <span>${escapeHtml(link.label || "Посилання")}</span>
        </a>
      `;
    }).join("");
    const logoUrl = getUsefulCardHomeUrl(card.links);
    const logoHtml = `
            <span class="useful-card-logo-wrap">
              <img src="${escapeHtml(card.logo || "")}" alt="" class="useful-card-logo" loading="lazy">
            </span>
    `;
    const logoBlock = logoUrl
      ? `<a class="useful-card-logo-link" href="${escapeHtml(logoUrl)}" target="_blank" rel="noopener noreferrer" aria-label="${escapeHtml(card.name || card.title || "Site")}">${logoHtml}</a>`
      : logoHtml;

    return `
      <article class="${cardClass} useful-card-${item.index + 1}">
        <div class="useful-card-header">
          ${logoBlock}
          <div>
            ${card.name ? `<div class="useful-card-name">${escapeHtml(card.name)}</div>` : ""}
            <h4 class="useful-card-title">${escapeHtml(card.title || "")}</h4>
            ${card.subtitle ? `<p class="useful-card-subtitle">${escapeHtml(card.subtitle)}</p>` : ""}
          </div>
        </div>
        ${phonesHtml ? `<div class="useful-phone-list">${phonesHtml}</div>` : ""}
        ${linksHtml ? `<div class="useful-actions">${linksHtml}</div>` : ""}
      </article>
    `;
  };

  const wideCards = renderedCards.filter(function (item) { return wideIndexSet.has(item.index); });
  const smallCards = renderedCards.filter(function (item) { return !wideIndexSet.has(item.index); });

  const wideHtml = wideCards.map(function (item, i) {
    return renderCard(item, i, "useful-card-wide");
  }).join("");
  const smallHtml = smallCards.map(function (item, i) {
    return renderCard(item, i, "useful-card-small");
  }).join("");

  const gridsHtml = [
    wideHtml ? `<div class="useful-links-grid useful-links-grid-main">${wideHtml}</div>` : "",
    smallHtml ? `<div class="useful-links-grid useful-links-grid-secondary">${smallHtml}</div>` : ""
  ].join("");

  root.innerHTML = `
    <details class="resident-useful-details useful-links-section">
      <summary class="useful-links-header"><span class="resident-title-icon" aria-hidden="true" data-lucide="info"></span><span>Корисні контакти та посилання</span></summary>
      <div class="useful-links-grid-wrap">${gridsHtml}</div>
    </details>
  `;
  root.style.display = "";
}
...
function getResidentFlatGridColumnCount(grid) {
  if (!grid) return 10;
  const raw = getComputedStyle(grid).getPropertyValue("--resident-flat-cols");
  const cols = Number(String(raw || "").trim());
  return Number.isFinite(cols) && cols > 0 ? Math.max(1, Math.round(cols)) : 10;
}

function residentFlatTileFits(tile) {
  if (!tile) return true;
  ...
  if (label) {
    ...
    const labelLineHeight = parseFloat(labelStyle.lineHeight) || ((parseFloat(labelStyle.fontSize) || 16) * 1.1);
    labelFits =
      label.scrollHeight <= (labelLineHeight * 2 + 2) &&
...
function layoutResidentFlatGrid(grid) {
  ...
  const spans = items.map(function (item) {
  ...
  items.forEach(function (item, idx) {
    ...
    for (; span <= cols; span++) {
      ...
      if (residentFlatTileFits(item)) break;
    }
    spans[idx] = Math.min(span, cols);
  });

  let col = 1;
  items.forEach(function (item, idx) {
    let span = Math.max(1, Math.min(spans[idx], cols));
    if (col + span - 1 > cols) {
      col = 1;
    }

    const remaining = cols - col + 1;
    const nextSpan = idx + 1 < spans.length ? Math.max(1, Math.min(spans[idx + 1], cols)) : 0;
    const isLastInRow = idx === spans.length - 1 || nextSpan > (remaining - span);
    if (isLastInRow) {
      span = remaining;
    }
...
function scheduleResidentFlatGridLayout(grid) {
  if (!grid) return;
  const run = function () {
    layoutResidentFlatGrid(grid);
  };
  requestAnimationFrame(run);
  setTimeout(run, 80);

  if (!window.__residentFlatGridResizeHandler) {
    window.__residentFlatGridResizeHandler = function () {
      document.querySelectorAll(".resident-flat-grid").forEach(layoutResidentFlatGrid);
    };
    window.addEventListener("resize", window.__residentFlatGridResizeHandler);
  }
}
...
function getMonthsForPaymentRange(accountId, paymentSum, paymentDate) {
  if (!accountId || !paymentDate || !nach[accountId]) return "";
  var paymentDateParts = String(paymentDate).split(".");
  ...
  for (var key in nach[accountId]) {
    if (Object.prototype.hasOwnProperty.call(nach[accountId], key)) years.push(key);
  }
  ...
  if (oplat[accountId]) {
    for (var y in oplat[accountId]) {
      for (var m in oplat[accountId][y]) {
        var paymentsBefore = oplat[accountId][y][m];
        for (var i = 0; i < paymentsBefore.length; i++) {
          var payment = paymentsBefore[i];
          var p = String(payment.date || "").split(".");
          var pDay = Number(p[0]);
          var pMonth = Number(p[1]);
          var pYear = Number(p[2]);
          if (
            pYear < year ||
            (pYear === year && pMonth < month) ||
            (pYear === year && pMonth === month && pDay < day)
          ) {
            remainingSum += Number(payment.sum) || 0;
          }
        }
      }
    }
  ...
  for (var yi = 0; yi < years.length; yi++) {
    var yy = years[yi];
    var shortYear = String(yy).slice(-2);
    var months = [];
    for (var monthKey in nach[accountId][yy]) {
      if (Object.prototype.hasOwnProperty.call(nach[accountId][yy], monthKey)) months.push(monthKey);
    }
    months.sort(function (a, b) { return a - b; });

    for (var mi = 0; mi < months.length; mi++) {
      var mm = months[mi];
      var charges = Object.values(nach[accountId][yy][mm]);
      var monthCharge = 0;
      for (var ci = 0; ci < charges.length; ci++) monthCharge += Number(charges[ci]) || 0;
      remainingSum -= monthCharge;
      if (remainingSum < -0.01 && !start) start = String(mm).padStart(2, "0") + "." + shortYear;
      if (monthCharge > 0 && remainingSum + (Number(paymentSum) || 0) < 0.01) {
        end = String(mm).padStart(2, "0") + "." + shortYear;
...
function buildResidentDesktopYearCards(yearPayload) {
  ...
  const getMonthAccrualSum = function (accountId, monthInt) {
    if (!accountId || !monthInt || !nach || !nach[accountId]) return 0;
    const y = Math.floor((Number(monthInt) - 1) / 12);
    const m = Number(monthInt) - y * 12;
    const monthData = nach[accountId] && nach[accountId][String(y)] && nach[accountId][String(y)][String(m)];
    if (!monthData || typeof monthData !== "object") return 0;
    return Object.values(monthData).reduce(function (sum, v) { return sum + (Number(v) || 0); }, 0);
  };

  const getThreeMonthAccrualLimit = function (accountId, year, month) {
    const monthInt = Number(year) * 12 + Number(month);
    if (!Number.isFinite(monthInt)) return 0;
    let total = 0;
    for (let i = 0; i < 3; i += 1) {
      total += getMonthAccrualSum(accountId, monthInt - i);
    }
    return total;
  };

  const table = document.createElement("table");
  table.className = "resident-desktop-history-table";
  table.innerHTML = `
    <thead>
      <tr>
        <th>Місяць</th>
  ...
  const appendDesktopMonthRow = function (month) {
    ...
    const meta = getMobileBalanceMeta(month.balance, !!month.isCurrent, month.year, month.month);
    const chargeLines = (month.charges || [])
      .filter(function (c) { return Number(c.value) !== 0; })
      .map(function (c) {
        const name = escapeHtml(String(c.name || ""));
        const value = (Number(c.value) || 0).toFixedWithComma();
        const detailRaw = String(c && c.detail || "").trim();
        if (!detailRaw) {
          return `<div class="rhd-line"><span>${name}</span><strong>${value}</strong></div>`;
        }
        const detailHtml = escapeHtml(detailRaw).replace(/\n/g, "<br>");
        return `
          <div class="rhd-line rhd-line-with-detail">
            <button type="button" class="rhd-detail-toggle" aria-expanded="false">
              <span>${name}</span>
              <span class="rhd-detail-icon">\u24D8</span>
            </button>
            <strong>${value}</strong>
            <div class="rhd-line-detail" hidden>${detailHtml}</div>
          </div>
        `;
      }).join("");

    const payments = Array.isArray(month.payments) ? month.payments : [];
    const paidHtml = payments.length
    ...
    const stateLabel = meta.cls === "debt"
      ? (isDue ? "До сплати" : "Борг")
      : (meta.cls === "credit" ? "Переплата" : "Стан");
    ...
    tr.innerHTML = `
      ...
      <td class="rhd-result rhd-${state.className}">${state.showLabel ? `<span class="rhd-state-chip">${state.label}</span>` : ""}<strong class="rhd-state-amount">${absBalance}</strong></td>
    `;
    if (month.isCurrent) tr.classList.add("rhd-current-month");
    tbody.appendChild(tr);

    if (Array.isArray(month.notes) && month.notes.length) {
      const lines = month.notes
        .filter(function (n) { return String(n || "").trim().length > 0; })
        .map(function (n) {
          const noteText = String(n || "")
            .trim()
            .replace(/^[\u2022•\-\s]+/, "");
          return `<div class="tarif-note-line">${escapeHtml(noteText)}</div>`;
        })
        .join("");
      if (lines) {
        tr.classList.add("rhd-has-note");
        const noteRow = document.createElement("tr");
        noteRow.className = "rhd-note-row";
        noteRow.innerHTML = `<td colspan="4"><div class="rhd-notes">${lines}</div></td>`;
        tbody.appendChild(noteRow);
      }
    }
  };

  closedMonths.forEach(appendDesktopMonthRow);
  ...
  if (summary) {
    ...
    totalRow.className = "rhd-total-row";
    totalRow.innerHTML = `
      ...
      <td class="rhd-result rhd-${totalState.className}">${totalState.showLabel ? `<span class="rhd-state-chip">${totalState.label}</span>` : ""}<strong class="rhd-state-amount">${totalAbs}</strong></td>
    `;
    tbody.appendChild(totalRow);
  }

  currentMonths.forEach(appendDesktopMonthRow);
...
//container.appendChild(yearContent);
    ...
    if (isResidentMode) {
      yearContent.appendChild(buildResidentDesktopYearCards(yearMobilePayload));
    }
...
if (isResidentMode && currentMonthPreliminaryShown) {
  ...
  historyNote.textContent = `* За ${historyNoteMonthName} ${currentYear} р. показано попередній розрахунок. Остаточна сума може змінитися.`;
...
if (toggleToOpen) {
  ...
  if (isResidentMode && residentOverviewRoot) {
    ...
    if (residentUsefulRoot) {
      renderResidentUsefulLinks(residentUsefulRoot);
    }
    ...
    if (residentSocialRowRoot) {
      ...
      const contactCardCount = Array.isArray(contactGroups) ? contactGroups.length : 0;
      ...
      residentSocialRowRoot.classList.toggle("contacts-full-width", hasViberBlock && hasContactsBlock && contactCardCount > 1);

      const applyResidentSocialRowLayout = function () {
        if (!residentSocialRowRoot) return;
        residentSocialRowRoot.classList.remove("social-force-stack");
        if (!(hasViberBlock && hasContactsBlock)) return;
        if (!residentViberRoot || !residentContactsRoot) return;
        const vTop = residentViberRoot.offsetTop;
        const cTop = residentContactsRoot.offsetTop;
        const wrapped = cTop > vTop + 1;
        residentSocialRowRoot.classList.toggle("social-force-stack", wrapped);
      };

      if (!window.__residentSocialRowLayoutHandler) {
        window.__residentSocialRowLayoutHandler = function () {
          if (typeof window.__residentSocialRowLayoutApply === "function") {
            window.__residentSocialRowLayoutApply();
          }
        };
        window.addEventListener("resize", window.__residentSocialRowLayoutHandler);
      }
      window.__residentSocialRowLayoutApply = applyResidentSocialRowLayout;
      requestAnimationFrame(applyResidentSocialRowLayout);
      setTimeout(applyResidentSocialRowLayout, 60);
    }
...
<div id="ls-picker" class="ls-picker hidden">
    <div class="ls-picker-header">
        ...
        <button id="ls-picker-close">&times;</button>
    </div>
      ...
      <div class="header-row">
        <div class="header-left">
          ...
          <div class="line ${isResidentMode ? "line-with-icon" : ""}">
            ...
            ${isResidentMode ? '<button type="button" id="residentSaveAccessBtn" class="resident-save-access-btn"><span class="resident-save-access-btn-icon" aria-hidden="true" data-lucide="bookmark"></span><span>Зберегти доступ</span></button>' : ""}
          </div>
...
document.addEventListener("click", function (e) {
    const rhdDetailBtn = e.target.closest(".rhd-detail-toggle");
    if (rhdDetailBtn) {
      const wrap = rhdDetailBtn.closest(".rhd-line-with-detail");
      const body = wrap ? wrap.querySelector(".rhd-line-detail") : null;
      if (!body) return;
      const isOpen = !body.hasAttribute("hidden");
      if (isOpen) {
        body.setAttribute("hidden", "");
        rhdDetailBtn.setAttribute("aria-expanded", "false");
      } else {
        body.removeAttribute("hidden");
        rhdDetailBtn.setAttribute("aria-expanded", "true");
      }
      return;
    }
