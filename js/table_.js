...
function generateTable() {
    ...
    for (const accountId in nach) {
        ...
        row.innerHTML = `<td>${ls[accountId].kv}</td><td>${debitStart.toFixed(2)}</td>`;
        ...
        if (displayMode === 'summarized') {
            ...
            Array.from(servicesWithCharges).forEach(serviceId => {
                ...
                row.innerHTML += `<td>${charge.toFixed(2)}</td>`;
            });
            ...
            row.innerHTML += `<td>${debitEnd.toFixed(2)}</td>`;
        } else {
            ...
            while (currentDate <= end) {
                ...
                row.innerHTML += `<td>${charges.toFixed(2)}</td>`;
                row.appendChild(generatePaymentCell(payments));
    ...
    footerRow.innerHTML = `<td>Итого</td><td>${totalStartDebt.toFixed(2)}</td>`;
    if (displayMode === 'summarized') {
        Array.from(servicesWithCharges).forEach(serviceId => {
            const serviceTotal = totalCharges[serviceId] || 0;
            footerRow.innerHTML += `<td>${serviceTotal.toFixed(2)}</td>`;
        });
        footerRow.innerHTML += `<td>${Object.values(totalPayments).reduce((sum, val) => sum + val, 0).toFixed(2)}</td><td>${totalEndDebt.toFixed(2)}</td>`;
    } else {
        let currentDate = new Date(start);
        while (currentDate <= end) {
            const month = currentDate.getMonth() + 1;
            ...
            footerRow.innerHTML += `<td>${chargeTotal.toFixed(2)}</td><td>${paymentTotal.toFixed(2)}</td>`;
            currentDate.setMonth(currentDate.getMonth() + 1);
        }
        footerRow.innerHTML += `<td>${totalEndDebt.toFixed(2)}</td>`;
    }
...
if (payments.length === 1) {
        ...
        if (payment.date) {
            paymentCell.innerHTML = `${totalPayment.toFixed(2)}
                <div class="descr">
